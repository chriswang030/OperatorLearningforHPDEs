...
%   * (optional) worker   : # worker for CONSTRUCTPAR = 0
%
%   Output:
...
% setup
mvs = zeros(1,maxlevel+1);      % total matvecs used
...
while level <= maxlevel && redvol > tol^2
    mvslevel = 0;                       % track matvecs this level
    ...
    for i = 1:lenr
        coord = reds(i,1:4);            % get coordinates of a red block
        ...
        for j = 1:16
            ch = children(j,:);     % get coordinates of child
            ...
            if q > 0
                ...
                mvslevel = mvslevel + mvsrd;
            else
                [U,S,V,mvssvd] = rsvd(Gch,k,p,q,W,W);
                high = S(1,1) > 1e-12 && S(k,k) > C(ic)*tol*S(1,1);
                mvslevel = mvslevel + mvssvd;
            end
    ...
    % update matvecs and errors
    mvs(level+1) = mvslevel;
    err = err + errlevel;
    ...
    if verbose
        fprintf("-------------------------\nLevel %d (worker: %d)\n\n" + ...
            ...
            level,worker,mvslevel,sum(mvs),lenr,redvol,errlevel,err);
    end
...
% compute L2 error from green blocks
errg = sqrt(err);

% compute L2 error from remaining red blocks
chlen = 2^(-level);
lenr = size(reds,1);
for i = 1:lenr
    ch = reds(i,1:4);   % get coordinates of red block
